...
__global__ void TurnOffMaximumEdgeOfConflictTriangles(int numberOfRows,
    ...
    for (int i = triangle_row_offsets_array_dev[maxA]; i < triangle_row_offsets_array_dev[maxA+1]; ++i){
        int a_tmp = triangle_candidates_a_dev[i];   
        int b_tmp = triangle_candidates_b_dev[i];    
        if ((a_tmp == v && b_tmp == maxB) || (a_tmp == maxB && b_tmp == v)){
            triangle_candidates_a_dev[i] = -1;
            triangle_candidates_b_dev[i] = -1;
            break;
        }
    }
    for (int i = triangle_row_offsets_array_dev[maxB]; i < triangle_row_offsets_array_dev[maxB+1]; ++i){
        int a_tmp = triangle_candidates_a_dev[i];   
        int b_tmp = triangle_candidates_b_dev[i];    
        if ((a_tmp == v && b_tmp == maxA) || (a_tmp == maxA && b_tmp == v)){
            triangle_candidates_a_dev[i] = -1;
            triangle_candidates_b_dev[i] = -1;
            break;
        }
...
// Each node maintains a list of the triangles it belongs to
// by the time this kernel is called, all of the nodes triangles
// except for one or zero will be (-1,-1).
// If this vertex is the largest index vertex of the triangle
// color the other two vertices with its own color.
...
void ColorGraph(){
    ...
    int * global_vertex_finished; // size N, contatin boolean of color finished, sometimes cardinality 1,2,3 is finished.
    ...
    cudaMalloc( (void**)&global_vertex_finished, numberOfRows * sizeof(int) );
    cudaMalloc( (void**)&finished_gpu, 1 * sizeof(int) );
    ...
    cuMemsetD32(reinterpret_cast<CUdeviceptr>(global_vertex_finished),  0, size_t(numberOfRows));
    ...
    for(int n=0; n<numberOfRows; ++n){
        ...
        cudaMemcpy(&new_vertex_finished[0], &global_vertex_finished[0], numberOfRows * sizeof(int) , cudaMemcpyDeviceToHost);
                ...
                for (int j = new_row_offs_host[i]; j < new_row_offs_host[i+1]; ++j){
                    if (i < new_cols_host[j]){
                        ...
                        if(nodeIt2 == nodeMap.end()) {
                            ...
                            if(new_vertex_finished[new_colors[new_cols_host[j]]]){
                ...
                for (int i = 0; i < numberOfRows; ++i){
                    if (host_U[i] == depth){
                        ...
                        if(nodeIt2 == predMap.end()) {
                            ...
                            if(new_vertex_finished[new_colors[w]]){
...
//    cudaMemcpy(&new_cols[0], &global_columns_dev_ptr[0], numberOfEdgesPerGraph * sizeof(int) , cudaMemcpyDeviceToHost);
    ...
    cudaFree( global_vertex_finished );
...
void PerformMSBFS(int numberOfRows,
        ...
        do {
            cuMemsetD32(reinterpret_cast<CUdeviceptr>(finished_gpu),  1, size_t(1));
            cuMemsetD32(reinterpret_cast<CUdeviceptr>(global_marked),  0, size_t(numberOfRows));
            cudaDeviceSynchronize();
            checkLastErrorCUDA(__FILE__, __LINE__);
            launch_gpu_connect_kernel<<<oneThreadPerNode,threadsPerBlock>>>(
                                    numberOfRows,
                                    global_marked,
                                    global_row_offsets_dev_ptr,
                                    global_columns_dev_ptr,
            ...
            // Current isnt incremented yet, curr == source.
            launch_gpu_connect_color_kernel<<<oneThreadPerNode,threadsPerBlock>>>(numberOfRows,
                                                                        global_marked,
                                                                        global_colors_dev_ptr,
                                                                        global_vertex_finished_dev_ptr,
                                                                        global_predecessors);
...
__global__ void multiply_distance_by_finished_boolean(int N,
    ...
    int vc = colors[v];
    M[v] = vertex_finished[vc]*U[v] + vertex_finished[vc]*U_Prev[v]; 
}
...
__global__ void launch_gpu_connect_color_kernel( int N, int *marked,
    ...
    if (marked[v]) {
        ...
        // un-finished color claims the predecessor.
        colors[vertexToClaim] = colors[v];
        color_cardinalities[v] = 1; 
        color_cardinalities[vertexToClaim] = 1;
    }
}
...
__global__ void launch_gpu_color_finishing_kernel_1( int N,
    ...
    if (color_card[cv] == 3 && !vertex_finished[cv]) {
        // iterate over neighbors
        int num_nbr = nodes[v+1] - nodes[v];
        int * nbrs = & edges[ nodes[v] ];
        for(int i = 0; i < num_nbr; i++) {
...
__global__ void launch_gpu_color_finishing_kernel_2( int N,
    ...
    if (middle_vertex[v] && color_card[cv] == 3 && !vertex_finished[cv]) {
        // iterate over neighbors
        int num_nbr = nodes[v+1] - nodes[v];
        int * nbrs = & edges[ nodes[v] ];
        for(int i = 0; i < num_nbr; i++) {
        ...
        vertex_finished[cv] = foundCycle;
    } else if (color_card[cv] == 4){
        vertex_finished[cv] = true;
    }
}
...
__global__ void launch_gpu_sssp_coloring_1(int N,
    ...
    if (!vertex_finished[wc] && !vertex_finished[vc]){
        colors[w] = colors[v];
    }
}
...
__global__ void launch_gpu_sssp_coloring_maximize(int N,
    ...
    if (vcc > wcc && !vertex_finished[vc] && !vertex_finished[wc]){
        colors[w] = colors[v];
        M[w] = 1;
    }
}
