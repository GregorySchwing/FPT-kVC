...
TEST(InduceGraphTest, InduceGraphTest) {
	...
	std::vector<int> newRowOff = gPrime.GetNewRowOffRef();
	std::vector<int> newCols = gPrime.GetNewColRef();
	std::vector<int> newVals = gPrime.GetNewValRef();
	...
	vector<int> testRowOff (rowOff, rowOff + sizeof(rowOff) / sizeof(rowOff[0]) );
	vector<int> testCols (colInd, colInd + sizeof(colInd) / sizeof(colInd[0]) );
	vector<int> testVals (colInd, colInd + sizeof(colInd) / sizeof(colInd[0]) );

	EXPECT_EQ(newRowOff, testRowOff);	
	EXPECT_EQ(colInd, testCols);	
	EXPECT_EQ(vals, testVals);	
}
